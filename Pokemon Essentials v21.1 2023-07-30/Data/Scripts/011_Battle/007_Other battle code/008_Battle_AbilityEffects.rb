...
Battle::AbilityEffects::EndOfRoundEffect.add(:ATTACKBOOST,
  proc { |ability, battler, battle|
    # A Pokémon's turnCount is 0 if it became active after the beginning of a
    # round
    if battler.turnCount > 0 && battle.choices[battler.index][0] != :Run &&
...
Battle::AbilityEffects::OnSwitchIn.add(:TRICKYSURGE,
  proc { |ability, battler, battle, switch_in|
    next if battle.field.effects[PBEffects::TrickRoom] > 0
    battle.pbShowAbilitySplash(battler)
    battle.field.effects[PBEffects::TrickRoom] = 5
    battle.pbAnimation(:TRICKROOM, battler, battler.pbDirectOpposing)
