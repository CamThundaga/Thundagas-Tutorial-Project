...
ItemHandlers::UseFromBag.add(:RAINFLUTE, proc { |item|
  next 2
})
...
ItemHandlers::UseInField.add(:RAINFLUTE, proc { |item|
  pbUseItemMessage(item)
  if GameData::Weather.get($game_screen.weather_type).category == :Rain
    pbMessage(_INTL("You cleared the Rain weather with the Rain Flute."))
    $game_screen.weather(0, 1, 20)
  else
    pbMessage(_INTL("You played the Rain Flute and made it Rain!"))
    $game_screen.weather(1, 1, 20)
  end
  next true
})
...
ItemHandlers::UseOnPokemon.add(:HALFPOTION, proc { |item, qty, pkmn, scene|
