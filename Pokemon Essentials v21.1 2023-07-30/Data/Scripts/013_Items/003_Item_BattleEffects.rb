...
ItemHandlers::CanUseInBattle.add(:RAINFLUTE, proc { |item, pokemon, battler, move, firstAction, battle, scene, showMessages|
  next true
})
...
ItemHandlers::CanUseInBattle.copy(:POTION, :HALFPOTION,
...
ItemHandlers::UseInBattle.add(:RAINFLUTE, proc { |item, battler, battle|
  battle.pbDisplay(_INTL("You played the Rain Flute!"))
  if battle.field.weather != :Rain
    battle.field.weather = :Rain
    battle.field.weatherDuration = 5
    weather_data = GameData::BattleWeather.try_get(:Rain)
    battle.pbCommonAnimation(weather_data.animation) if weather_data
    battle.pbDisplay(_INTL("It started raining!"))
  else
    battle.field.weather = :None
    battle.field.weatherDuration = 0
    battle.pbDisplay(_INTL("It stopped raining!"))
  end
})
...
ItemHandlers::BattleUseOnPokemon.add(:HALFPOTION, proc { |item, pokemon, battler, choices, scene|
